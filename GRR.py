...
class GRR(object):
    ...
    def varComp(self, alpha = 0.25):
        ...
        if ps['p_Part_by_Operator'] > alpha:
            ...
            vc_reproducibility = vc_part_by_operator if vc_operator - vc_part_by_operator < 0 else vc_operator - vc_part_by_operator
            vc_total_grr = vc_repeatability + vc_reproducibility
            total_var = vc_total_grr + vc_parts          
            df = df_add(['Total Gage R&R', 'Repeatability', 'Reproducibility', 'Part * Operator', 'Operator', 'Part-to-Part', 'Total Variation'],
            [x if x>0 else 0 for x in [vc_total_grr, vc_repeatability, vc_reproducibility, vc_part_by_operator, vc_operator,  vc_parts, total_var]],
            [x if x>0 else 0 for x in np.array([vc_total_grr, vc_repeatability, vc_reproducibility, vc_part_by_operator, vc_operator,  vc_parts, total_var])/ total_var])
            self.vc_table = df
            return df
